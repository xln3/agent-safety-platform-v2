...
def parse_answers(state: TaskState, multiple_correct: bool) -> set[str]:
    ...
    # NOTE: Allow optional $ signs around the answer (e.g. "$C$") because the
    # prompt template uses $LETTER as a placeholder and some models reproduce
    # the dollar signs literally.
    match = re.search(
        r"(?i)^ANSWER\s*:\s*\$?\s*([A-Za-z\d ,]+)\s*\$?\s*(?:$|\n|\.)",
